...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (typeof facRemarks !== 'string') {
      return new NextResponse(JSON.stringify({ error: "Faculty remarks must be text" }), { 
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    const faculty = await db.collection("ics").findOne({ email: session.user.email });
    ...
    if (request.courseCode !== faculty.courseCode) {
      return new NextResponse(JSON.stringify({ error: "Not authorized to modify this request" }), { 
        status: 403,
        headers: { "Content-Type": "application/json" }
      });
    }
      ...
      if (request.evalComponent === "Mid Semester Exam") {
        ...
        if (request.email) {
          await sendEmail(
            ...
            `Your Mid Semester Exam Makeup Request has been ${status} by the faculty. Remarks: ${sanitizedRemarks}`
          );
        }
      } else if (request.evalComponent === "Comprehensive Exam") {
        if (status === "Approved") {
          // Update with faculty approved status
          await db.collection("makeup-requests").updateOne(
            { _id: new ObjectId(id) },
            { $set: { status: "faculty approved", facRemarks: sanitizedRemarks } }
          );
          ...
          if (ttdEmail) {
            await sendEmail(
              ...
              `A makeup request for End Semester Exam has been approved by faculty and requires your review. Student: ${request.name}, Course: ${request.course}`
            );
          }
          ...
          if (request.email) {
            await sendEmail(
              ...
              `Your End Semester Exam Makeup Request has been ${status} by the faculty. Remarks: ${sanitizedRemarks}`
            );
          }
        }
      }
